...
class ImageApp(tk.Tk):
    ...
    def _init_style(self):
        ...
        # Evaluation label style (slightly darker pink for readability)
        style.configure("Eval.TLabel",
                        background=bg_panel,
                        foreground="#b91c1c",
        ...
        # Styled horizontal scale to match accent (may look different per OS theme)
        style.configure("Accent.Horizontal.TScale",
                        troughcolor="#fde2e4",
                        background=accent,
                        troughrelief="flat")
        # value label style for sliders (pink pill, like zoom percentage)
        style.configure("SliderValue.TLabel",
                        background=accent,
                        foreground="white",
                        font=("Segoe UI", 9, "bold"))
    ...
    def _build_basic_tab(self, notebook):
        ...
        # ---------- Member 2 UI (buttons, evaluation, sliders) ----------
        ...
        btn_row.pack(fill=tk.X, pady=(6, 4), padx=6)

        ttk.Button(btn_row, text="Grayscale", command=self.apply_grayscale).pack(
            side=tk.LEFT, padx=(0,6), pady=2
        )
        ttk.Button(btn_row, text="Binary (mean threshold)", command=self.apply_binary).pack(
            side=tk.LEFT, padx=(0,6), pady=2
        )
        ttk.Button(btn_row, text="Threshold", command=self.apply_thresh).pack(
            side=tk.LEFT, padx=(0,6), pady=2
        )

        # ---------------- Evaluation area (full width under buttons) ----------------
        eval_frame = ttk.Frame(proc_frame)
        eval_frame.pack(fill=tk.X, padx=8, pady=(6,6))

        ttk.Label(eval_frame, text="Evaluation:", style="Eval.TLabel").pack(anchor="w")
        self.binary_comment_var = tk.StringVar(value="")
        # comment_label uses Eval.TLabel style, wraps and expands across available width
        comment_label = ttk.Label(
            eval_frame,
            textvariable=self.binary_comment_var,
            style="Eval.TLabel",
            anchor="w",
            justify="left"
        )
        comment_label.pack(fill=tk.X, pady=(2,0))

        # ---------------- Sliders area (each on its own row, with 0 & 255 markers) ----------
        sliders_frame = ttk.Frame(proc_frame)
        sliders_frame.pack(fill=tk.X, padx=6, pady=(8,6))

        # --- Max Value slider row ---
        self.thresh_maxval = tk.IntVar(value=255)
        self.thresh_maxval_display = tk.StringVar(value=str(self.thresh_maxval.get()))
        max_row = ttk.Frame(sliders_frame)
        max_row.pack(fill=tk.X, pady=4)

        # left label "0"
        ttk.Label(max_row, text="0", width=4).pack(side=tk.LEFT, padx=(6,4))

        # label title
        ttk.Label(max_row, text="Max Value:", width=12).pack(side=tk.LEFT)

        # slider with range 0..255
        max_scale = ttk.Scale(max_row, from_=0, to=255, orient=tk.HORIZONTAL,
                              variable=self.thresh_maxval, style="Accent.Horizontal.TScale",
                              command=lambda v: self.thresh_maxval_display.set(f"{int(float(v))}"))
        max_scale.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(6,6))

        # right-end label "255"
        ttk.Label(max_row, text="255", width=4).pack(side=tk.RIGHT, padx=(6,6))

        # live value badge (pink pill)
        ttk.Label(max_row, textvariable=self.thresh_maxval_display, style="SliderValue.TLabel", width=6).pack(side=tk.RIGHT, padx=(4,0))

        # --- Threshold Value slider row ---
        self.thresh = tk.IntVar(value=128)
        self.thresh_display = tk.StringVar(value=str(self.thresh.get()))
        thresh_row = ttk.Frame(sliders_frame)
        thresh_row.pack(fill=tk.X, pady=4)

        # left label "0"
        ttk.Label(thresh_row, text="0", width=4).pack(side=tk.LEFT, padx=(6,4))

        # label title
        ttk.Label(thresh_row, text="Threshold Value:", width=12).pack(side=tk.LEFT)

        thresh_scale = ttk.Scale(thresh_row, from_=0, to=255, orient=tk.HORIZONTAL,
                                 variable=self.thresh, style="Accent.Horizontal.TScale",
                                 command=lambda v: self.thresh_display.set(f"{int(float(v))}"))
        thresh_scale.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(6,6))

        # right-end label "255"
        ttk.Label(thresh_row, text="255", width=4).pack(side=tk.RIGHT, padx=(6,6))

        # live value badge (pink pill)
        ttk.Label(thresh_row, textvariable=self.thresh_display, style="SliderValue.TLabel", width=6).pack(side=tk.RIGHT, padx=(4,0))
    ...
    def reset_image(self):
        ...
        # Update image info (resolution/size/type) using current_image/current_array
        # _update_image_info() will prefer file-based info if a filename was passed,
        # but when called without args it will read from self.current_image/self.current_array.
        try:
            self._update_image_info()
        except Exception:
            # Never let an info update error stop the UI reset
            pass
    ...
    def _update_image_info(self, filename: str = None):
        """
        Update the info_text shown in the UI.
        If filename is provided, attempt to read file info (width, height, file size, format).
        Otherwise, if self.current_image is available, read resolution from that PIL Image
        and show approximate size (in bytes) and image mode/type.
        This enables real-time updates when operations change the displayed image.
        """
        try:
            if filename:
                # Prefer file-based info if a filename is given (original file on disk)
                width, height, size_str, img_type = m1_get_info(filename)
                self.info_text.set(f"Resolution: {width}x{height}    Size: {size_str}    Type: {img_type}")
                return

            # No filename -> use current_image (live)
            if getattr(self, "current_image", None) is None:
                self.info_text.set("Resolution: -    Size: -    Type: -")
                return

            # current_image is a PIL.Image
            pil = self.current_image
            width, height = pil.size
            img_mode = pil.mode or "Unknown"
            # Estimate memory size in bytes from array if available
            if getattr(self, "current_array", None) is not None:
                size_bytes = int(self.current_array.nbytes)
                # Display human readable using existing helper-like logic
                ...
                while s >= 1024.0 and idx < len(units)-1:
                    s /= 1024.0
                    idx += 1
                size_str = f"{int(s)} {units[idx]}" if idx == 0 else f"{s:.1f} {units[idx]}"
            else:
                # fallback: approximate by saving to bytes (fast for small images)
                try:
                    import io
                    buf = io.BytesIO()
            ...
            self.info_text.set(f"Resolution: {width}x{height}    Size: {size_str}    Type: {img_mode}")
    ...
    def _set_array_as_image(self, arr, mode="auto"):
        """
        Set a numpy array as the current image (PIL). Also update the current_array and
        call _update_image_info() so the GUI's Image Info updates in real-time.
        """
        if arr is None:
            return
        arr = np.asarray(arr)
        ...
        # Update both image and array, choose shown image, then refresh display & info
        self.current_image = img
        self.current_array = arr
        self.chosen_image = self.current_image

        # Update image info (realtime resolution / size / type)
        try:
            self._update_image_info()
        except Exception:
            # Don't let info update errors block display
            pass

        # Redraw viewer after the info update
        self._update_image_display()
    ...
    def compress_image(self):
        ...
        # Create image and use _set_array_as_image so info updates automatically
        try:
            arr2 = compressed_arr.reshape(h, w)
            self._set_array_as_image(arr2, mode="L")
        except Exception:
            # fallback to previous approach if reshape fails
